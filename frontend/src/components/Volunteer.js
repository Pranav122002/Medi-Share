...
export default function Volunteer() {
  ...
  function fetchUnverifiedOrders() {
    fetch(`/unverifiedorders/${VolunteerId}`, {
      method: "GET",
      headers: {
        Authorization: "Bearer " + localStorage.getItem("jwt"),
      },
  ...
  const delivery_order = (order_id) => {
    fetch(`/delivery-executed/${order_id}`, {
      method: "put",
    }).then((res) => {});
  };

  const verify_donate_order = (order_id) => {
    fetch(`/verify-donate-order/${order_id}`, {
      method: "put",
      headers: {
        Authorization: "Bearer " + localStorage.getItem("jwt"),
      },
  ...
  const verify_request_order = async (order_id) => {
    try {
      const response = await fetch(`/req-order/${order_id}`, {
        method: "GET",
        headers: {
          Authorization: "Bearer " + localStorage.getItem("jwt"),
        },
      ...
      if (order_data.isDonarFieldBlank === false) {
        fetch(`/verify-request-order/${order_id}`, {
          method: "put",
          headers: {
            Authorization: "Bearer " + localStorage.getItem("jwt"),
          },
  ...
  const handleAccept = (order) => {
    fetch(`/volunteer-accept/${order.unverifiedorders._id}`, {
      method: "put",
      headers: {
  ...
  const handleReject = (order) => {
    fetch(`/volunteer-reject/${order.unverifiedorders._id}`, {
      method: "put",
      headers: {
        "Content-Type": "application/json",
      },
