  ...
  const sendMessage = async (message) => {
    ...
    try {
      ...
      || response.data.message === "4" 
        ...
        const discriptions = diseases
        const precautions = diseases
        const medicines = diseases
        // Send symptoms to FastAPI
        ...
        const botMessage = {
          sender: 'bot',
          content: `
            ${diseaseResponse.data.message}, 
            ${precautionResponse.data.message}
            ${medicines.data.message}
            ${discriptionResponse.data.message}
            `
