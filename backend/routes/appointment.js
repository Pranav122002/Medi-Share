...
router.post("/book-appointment/:id", async (req, res, next) => {
    try {
      ...
      if (credits > 200) {
        ...
        if (data) {
          return res.json({ msg: "Appointment placed successfully. The doctor will confirm now..." });
        } else {
          return res.json({ msg: "Failed to place the appointment..." });
        }
      } else {
        return res.json({ msg: "Insufficient credits. Please earn your credits to book an appointment..." });
      }
    } catch (ex) {
      next(ex);
    }
