...
router.get("/my-doctor-appointments/:id", async (req, res, next) => {
  try {
...
router.get("/my-patient-appointments/:id", async (req, res, next) => {
  try {
    const appointments = await APPOINTMENT.find({
...
router.get("/doctor-appointments/:id", async (req, res, next) => {
  try {
    const appointments = await APPOINTMENT.find({
      doctor: req.params.id,
    })
...
router.get("/patient-appointments/:id", async (req, res, next) => {
  try {
    const appointments = await APPOINTMENT.find({
      patient: req.params.id,
    })
...
router.put("/reject-appointment/:id", async (req, res, next) => {
  try {
...
router.put("/add-appointment-link/:id", async (req, res, next) => {
...
router.put("/add-rating-feedback/:id", async (req, res, next) => {
...
router.get("/doctor-ratings-feedbacks/:id", async (req, res, next) => {
  try {
    const appointments = await APPOINTMENT.find({
      doctor: req.params.id,
